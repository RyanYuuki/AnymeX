...
class _NovelReadingPageState extends State<NovelReadingPage> {
  ...
  late NovelSourcesHandler novelSourcesHandler;
  ...
  Future<void> _fetchInitialChapter() async {
    try {
      final data = await novelSourcesHandler.fetchNovelWords(widget.id);
      if (mounted) {
        setState(() {
          novelData = data;
          _isLoading = false;
  ...
  Future<void> _fetchNextPreviousChapter(String chapterId) async {
    ...
    try {
      final data = await novelSourcesHandler.fetchNovelWords(chapterId);
      if (mounted) {
        setState(() {
          novelData = data;
