...
class _NovelContentWidgetState extends State<NovelContentWidget> {
  ...
  bool get _isDragRecent => _lastDragEnd == null
      ? false
      : DateTime.now().difference(_lastDragEnd!).inMilliseconds < 350;
        ...
        onVerticalDragEnd: (details) {
          ...
          if (widget.controller.swipeGestures.value) {
            if (details.primaryVelocity != null) {
              ...
              final maxScroll = widget.controller.scrollController.hasClients
...
class _DictionarySheetState extends State<_DictionarySheet> {
  ...
  Future<void> _lookup() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _error = 'Could not reach dictionary.';
          _loading = false;
        });
      }
    }
...
class _DictEntry {
  ...
  const _DictEntry(
      {required this.partOfSpeech, required this.definition, this.example});
}
