...
class NovelContentWidget extends StatefulWidget {
  ...
  const NovelContentWidget({super.key, required this.controller});
...
class _NovelContentWidgetState extends State<NovelContentWidget> {
  ...
  bool get _isDragRecent => _lastDragEnd == null ? false : DateTime.now().difference(_lastDragEnd!).inMilliseconds < 350;
    ...
    return SelectionArea(
      ...
      child: GestureDetector(
        ...
        onVerticalDragEnd: (details) {
          ...
          if (widget.controller.swipeGestures.value) {
            if (details.primaryVelocity != null) {
              ...
              final scrollPosition = widget.controller.scrollController.offset;
              final maxScroll = widget.controller.scrollController.hasClients ? widget.controller.scrollController.position.maxScrollExtent : 0;
              if (isScrollingDown && scrollPosition >= maxScroll - 20) {
                if (widget.controller.canGoNext.value) widget.controller.goToNextChapter();
              } else if (!isScrollingDown && scrollPosition <= 20) {
                if (widget.controller.canGoPrevious.value) widget.controller.goToPreviousChapter();
              }
            }
          }
        },
        ...
        child: Container(
          ...
          child: SafeArea(
            child: Obx(() {
              if (widget.controller.loadingState.value == LoadingState.loading) return _buildLoadingState(context);
              if (widget.controller.loadingState.value == LoadingState.error || widget.controller.novelContent.value.isEmpty) return _buildEmptyState(context);
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text('Loading chapter...', style: TextStyle(color: context.colors.onSurface.opaque(0.7), fontSize: 16)),
        ],
      ),
    );
  }
  ...
  Widget _buildEmptyState(BuildContext context) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Icon(Icons.book_outlined, size: 48, color: context.colors.onSurface.opaque(0.5)),
          const SizedBox(height: 16),
          Text('No content available', style: TextStyle(fontSize: 18, color: context.colors.onSurface.opaque(0.7), fontWeight: FontWeight.w500)),
        ],
      ),
    );
  }
  ...
  Widget? _getCustomWidget(dom.Element element, BuildContext context) {
    if (element.localName?.toLowerCase() == 'img') {
      return Padding(
        ...
        child: AnymeXImage(imageUrl: element.attributes['src']!, fit: BoxFit.contain, radius: 8),
      );
    }
    return null;
  }
  ...
  TextStyle _getBaseTextStyle(BuildContext context) {
    return TextStyle(
      ...
      color: widget.controller.useSystemReaderTheme ? Theme.of(context).textTheme.bodyLarge?.color : widget.controller.readerTextColor,
      fontFamily: widget.controller.fontFamilyName.isEmpty ? null : widget.controller.fontFamilyName,
  ...
  Map<String, String>? _getCustomStyles(dom.Element element, BuildContext context) {
    ...
    switch (element.localName?.toLowerCase()) {
      ...
      case 'h1': case 'h2': case 'h3': case 'h4': case 'h5': case 'h6':
        ...
        break;
      case 'blockquote':
        styles['margin'] = '${widget.controller.paragraphSpacing.value}px 24px';
        styles['padding'] = '8px 16px';
        styles['border-left'] = '4px solid #${context.colors.primary.value.toRadixString(16).padLeft(8, '0')}';
...
class _DictionarySheetState extends State<_DictionarySheet> {
  ...
  Future<void> _lookup() async {
    try {
      final uri = Uri.parse('https://api.dictionaryapi.dev/api/v2/entries/en/${Uri.encodeComponent(widget.word)}');
        ...
        for (final item in data) {
          for (final meaning in (item['meanings'] as List? ?? [])) {
            ...
            for (final def in (meaning['definitions'] as List? ?? []).take(2)) {
              entries.add(_DictEntry(partOfSpeech: pos, definition: def['definition'] as String? ?? '', example: def['example'] as String?));
            }
          }
        }
        if (mounted) setState(() { _entries = entries; _loading = false; });
      } else {
        if (mounted) setState(() { _error = 'No definition found for "${widget.word}"'; _loading = false; });
      }
    } catch (e) {
      if (mounted) setState(() { _error = 'Could not reach dictionary.'; _loading = false; });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Center(child: Container(width: 40, height: 4, decoration: BoxDecoration(color: colors.onSurface.withOpacity(0.2), borderRadius: BorderRadius.circular(2)))),
          const SizedBox(height: 16),
          Text(widget.word, style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: colors.primary)),
          const SizedBox(height: 12),
          if (_loading) const Center(child: CircularProgressIndicator())
          else if (_error != null) Text(_error!, style: TextStyle(color: colors.error))
          else ConstrainedBox(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, i) {
                ...
                return Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(e.partOfSpeech, style: TextStyle(fontSize: 12, fontStyle: FontStyle.italic, color: colors.onSurface.withOpacity(0.6))),
                    const SizedBox(height: 4),
                    Text(e.definition, style: const TextStyle(fontSize: 15)),
                    if (e.example != null) ...[const SizedBox(height: 4), Text('"${e.example}"', style: TextStyle(fontSize: 13, fontStyle: FontStyle.italic, color: colors.onSurface.withOpacity(0.55)))],
                  ],
                );
              },
            ),
...
class _DictEntry {
  final String partOfSpeech, definition;
  final String? example;
  const _DictEntry({required this.partOfSpeech, required this.definition, this.example});
}
