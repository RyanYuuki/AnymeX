...
class _NovelDetailsPageState extends State<NovelDetailsPage> {
  ...
  void _checkNovelPresence() {
    ...
    if (data.value.id != null || data.value.id != '') {
      isListedNovel.value = true;
      currentNovel = data;
    } else {
      isListedNovel.value = false;
  ...
  void _initListVars() {
    Logger.i('[_initListVars] ${currentNovel.value?.episodeCount}');
    novelProgress.value = currentNovel.value?.episodeCount?.toInt() ?? 0;
    novelScore.value = currentNovel.value?.score?.toDouble() ?? 0.0;
    novelStatus.value = currentNovel.value?.watchingStatus ?? "";
  }
  ...
  List<String> formatTitles(Media media) {
    return ['${media.title}*NOVEL', media.romajiTitle];
  }
  ...
  List<Chapter> _convertChapters(List<DEpisode> chapters, String title) {
    return chapters.map((e) {
      return Chapter(
        title: e.name,
        link: e.url,
        number: e.episodeNumber?.toDouble() ?? 0,
        releaseDate: e.dateUpload,
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: AnymexTextSpans(
                ...
                spans: [
                  ...
                  AnymexTextSpan(
                    text: novelProgress.value.toString(),
                    variant: TextVariant.bold,
            ...
            Container(
              ...
              child: Text(
                '${formatProgress(currentChapter: novelProgress.value, totalChapters: anilistData?.totalChapters ?? 1, altLength: chapterList?.length ?? 1)}%',
                style: TextStyle(
                  color: context.colors.primary,
                  fontSize: 12,
    ...
    showModalBottomSheet(
      ...
      builder: (BuildContext context) {
        return ListEditorModal(
          animeStatus: novelStatus,
          animeScore: novelScore,
          animeProgress: novelProgress,
          currentAnime: currentNovel,
          isManga: true,
