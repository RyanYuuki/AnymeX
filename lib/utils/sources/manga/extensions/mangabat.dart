...
class MangaBat implements SourceBase {
  @override
  ...
  @override
  String get baseUrl => 'https://h.mangabat.com/';

  @override
  Future<dynamic> fetchChapterImages(
      {required String mangaId, required String chapterId}) async {
    final url = 'https://readmangabat.com/$mangaId-chap-$chapterId';
    int index = 0;

    try {
      final response = await http
          .get(Uri.parse(url), headers: {'Referer': 'https://h.mangabat.com/'});
      if (response.statusCode == 200) {
        final document = parse(response.body);
        log(url);
