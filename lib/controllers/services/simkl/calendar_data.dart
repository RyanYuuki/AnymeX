  ...
  if (response.statusCode == 200) {
    ...
    List<Media> newMediaList = schedules
        .map<Media?>((schedule) {
          ...
          DateTime airDate;
          try {
            airDate = DateTime.parse(schedule['release_date'] ?? schedule['date'] ?? '');
          } catch (e) {
            return null;
          }
          
          media.nextAiringEpisode = NextAiringEpisode(
            airingAt: airDate.millisecondsSinceEpoch ~/ 1000,
            timeUntilAiring: airDate.difference(DateTime.now()).inSeconds,
            episode: 1,
          );
          
          media.aired = schedule['release_date'] ?? schedule['date'] ?? '';
