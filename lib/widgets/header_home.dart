...
class Header extends StatelessWidget {
  ...
  Widget _buildNonHomePageHeader(
          ...
          children: [
            Expanded(
              child: AnymexTextSpans(
                fontSize: 18,
                spans: [
                  const AnymexTextSpan(text: 'Hey ', variant: TextVariant.bold),
                  AnymexTextSpan(
                      text: profileData.isLoggedIn.value
                          ? profileData.profileData.value.name ?? 'Guest'
                          : 'Guest',
                      color: Theme.of(context).colorScheme.primary,
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      if (!isHomePage) {
        return _buildNonHomePageHeader(context, profileData);
      } else {
  ...
  AnymexOnTap _profileIcon(BuildContext context, ServiceHandler profileData) {
    return AnymexOnTap(
      ...
      child: CircleAvatar(
        radius: _iconAvatarRadius, // Using the constant
        backgroundColor: Theme.of(context)
            .colorScheme
            .secondaryContainer
            .withValues(alpha: 0.50),
