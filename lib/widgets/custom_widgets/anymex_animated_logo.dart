...
class _AnymeXAnimatedLogoState extends State<AnymeXAnimatedLogo>
  ...
  void initState() {
    ...
    // Badge rotation animation (2.5s, 180°)
    ...
    // Fill fade animation (0.5s at 2.3s)
  ...
  Future<void> _startAnimation() async {
    // Start badge rotation immediately
    _rotationController.forward();
    
    // Start stroke drawing after 500ms
    await Future.delayed(const Duration(milliseconds: 500));
    if (mounted) _strokeController.forward();
    
    // Start fill fade after 2300ms total
  ...
  Widget _buildBadge() {
    ...
    // Badge SVG string with gradient
    final badgeSvg = '''
  ...
  Widget _buildLogo() {
    ...
    // Logo SVG string with gradient and animations
    final logoSvg = '''
      <svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <linearGradient id="logoGradient" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stop-color="${_colorToHex(logoColor)}" />
            <stop offset="50%" stop-color="${_colorToHex(logoColor.withOpacity(0.8))}" />
            <stop offset="100%" stop-color="${_colorToHex(logoColor.withOpacity(0.6))}" />
          </linearGradient>
        </defs>
        <path
          ...
          stroke-dasharray="${_strokeAnimation.value < 1 ? '1000' : '0'}"
          stroke-dashoffset="${(1 - _strokeAnimation.value) * 1000}"
        />
      </svg>
    ''';
